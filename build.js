...
// src/sendMessage.js
async function sendMessage(pid, data, action) {
  const messageId = await connect().message({
    process: pid,
    signer: createDataItemSigner2(globalThis.arweaveWallet),
    tags: [{ name: "Action", value: action }],
    data
  });
  const result2 = await connect().result({
    message: messageId,
    process: pid
  });
  console.log(result2);
  if (result2.Error) {
    myUnityInstance.SendMessage("AOConnectManager", "MessageCallback", result2.Error);
    throw new Error(result2.Error);
  }
  if (result2.Output?.data?.json) {
    myUnityInstance.SendMessage("AOConnectManager", "MessageCallback", result2.Output?.data?.json);
    return result2.Output?.data?.json;
  }
  return void 0;
}

// src/spawnProcess.js
async function spawnProcess(name) {
...
var fetchProcessTransactionsQuery = (address) => {
  return `{
      transactions(
        first: 100
        owners: ["${address}"]
...
async function fetchProcesses(address) {
...
// src/dumdum.js
async function dumdumRegisterReferral(pid, username, referral) {
  const messageId = await connect().message({
    process: pid,
    signer: createDataItemSigner2(globalThis.arweaveWallet),
  ...
  console.log(result2);
  if (result2.Error) {
    myUnityInstance.SendMessage("SocialManager", "RegistrationCallback", result2.Error);
    throw new Error(result2.Error);
  }
  if (result2.Messages[0].Data) {
    myUnityInstance.SendMessage("SocialManager", "RegistrationCallback", result2.Messages[0].Data);
    return result2.Messages[0].Data;
  }
  return void 0;
}
async function dumdumRegister(pid, username) {
  const messageId = await connect().message({
    process: pid,
    signer: createDataItemSigner2(globalThis.arweaveWallet),
...
async function dumdumPet(pid) {
  ...
  console.log(result2);
  if (result2.Error) {
    myUnityInstance.SendMessage("SocialManager", "PetCallback", result2.Error);
    throw new Error(result2.Error);
  }
  if (result2.Messages[0].Data) {
    console.log(result2.Messages[0].Data);
    myUnityInstance.SendMessage("SocialManager", "PetCallback", result2.Messages[0].Data);
    return result2.Messages[0].Data;
  }
  return void 0;
}
async function dumdumVote(pid, data) {
  const messageId = await connect().message({
    process: pid,
    signer: createDataItemSigner2(globalThis.arweaveWallet),
  ...
  console.log(result2);
  if (result2.Error) {
    myUnityInstance.SendMessage("SocialManager", "VoteCallback", result2.Error);
    throw new Error(result2.Error);
  }
  if (result2.Messages[0].Data) {
    myUnityInstance.SendMessage("SocialManager", "VoteCallback", result2.Messages[0].Data);
    return result2.Messages[0].Data;
  }
  return void 0;
}
async function dumdumSendChat(pid, chatText) {
  const messageId = await connect().message({
    process: pid,
    signer: createDataItemSigner2(globalThis.arweaveWallet),
  ...
  console.log(result2);
  if (result2.Error) {
    myUnityInstance.SendMessage("SocialManager", "SendChatMessageCallback", result2.Error);
    throw new Error(result2.Error);
  }
  if (result2.Messages[0].Data) {
    myUnityInstance.SendMessage("SocialManager", "SendChatMessageCallback", result2.Messages[0].Data);
    return result2.Messages[0].Data;
  }
  return void 0;
}
...
function requestNotificationPermission() {
  if ("Notification" in window) {
    Notification.requestPermission().then(function(result2) {
      console.log("Notification permission: ", result2);
    });
  }
}
  ...
  if (!("Notification" in window)) {
    ...
    Notification.requestPermission(function(permission) {
      if (permission === "granted") {
        var notification2 = new Notification(title, { body: text });
        console.log("Notification sent");
      }
    });
  }
